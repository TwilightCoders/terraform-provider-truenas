...
// NFSShare manages NFS exports (sharing.nfs).
var NFSShare = engine.Resource{Type: "nfs_share", Namespace: "sharing.nfs"}

// StaticRoute manages static network routes (staticroute).
var StaticRoute = engine.Resource{Type: "static_route", Namespace: "staticroute"}

// Tunable manages sysctl, udev and ZFS module tunables (tunable).
var Tunable = engine.Resource{Type: "tunable", Namespace: "tunable"}

// ISCSIPortal manages iSCSI portals: the addresses targets listen on (iscsi.portal).
var ISCSIPortal = engine.Resource{Type: "iscsi_portal", Namespace: "iscsi.portal"}

// ISCSIInitiator manages groups of initiators allowed to connect (iscsi.initiator).
var ISCSIInitiator = engine.Resource{Type: "iscsi_initiator", Namespace: "iscsi.initiator"}

// ISCSITarget manages iSCSI targets (iscsi.target).
var ISCSITarget = engine.Resource{Type: "iscsi_target", Namespace: "iscsi.target"}

// ISCSIExtent manages iSCSI extents: the zvols or files a target exposes (iscsi.extent).
var ISCSIExtent = engine.Resource{
	Type:      "iscsi_extent",
	Namespace: "iscsi.extent",
	Fields: map[string]engine.Field{
		// Generated by TrueNAS when not given.
		"serial": engine.Computed,
	},
}

// ISCSITargetExtent associates an extent with a target as a LUN (iscsi.targetextent).
var ISCSITargetExtent = engine.Resource{Type: "iscsi_target_extent", Namespace: "iscsi.targetextent"}

// ISCSIAuth manages iSCSI authorized access credentials (iscsi.auth).
var ISCSIAuth = engine.Resource{
	Type:      "iscsi_auth",
